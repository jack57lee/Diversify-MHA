...
def _infer_shape_invariants(tensor):
    ...
    #for visualization (Baosong)
    if len(shape)==6:
        return tf.TensorShape([None,None,None,None,None,None])
    else:
...
def beam_search(func, state, batch_size, beam_size, max_length, alpha,
    ...
    # for visualization (Baosong)						
    return final_seqs, final_scores, final_state.state[0]
...
def create_inference_graph(model_fns, features, params):
    ...
    states = nest.map_structure(lambda x: _tile_to_beam_size(x, beam_size),
                                states)
    # for visualization (Baosong)    
    seqs, scores, states = beam_search(decoding_fn, states, batch_size, beam_size,
                               max_length, alpha, pad_id, bos_id, eos_id)
    # for visualization (Baosong)
    if params.outweights: 
        return seqs[:, :top_beams, 1:], scores[:, :top_beams], states["enc_atts"][:,0,:,:,:,:], states["dec_atts"][:,0,:,:,:,:], states["encdec_atts"][:,0,:,:,:,:]
    else: 
