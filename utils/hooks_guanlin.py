...
def _evaluate(eval_fn, input_fn, decode_fn, path, is_Reverse, is_BPE, global_step,  config):
    graph = tf.Graph()
    with graph.as_default():
        ...
        if is_Reverse:
            for sent in decoded_symbols:
                sent.reverse()
        if is_BPE:
            save_path_bpe = os.path.join(path, 'model.ckpt-' + str(global_step) + '.pred.bpe')
            save_path_norm = os.path.join(path, 'model.ckpt-' + str(global_step) + '.pred.norm')
            ...
            save_path_norm = os.path.join(path, 'model.ckpt-' + str(global_step) + '.pred.norm')
            with open(save_path_norm, 'w') as f:
                for sent in decoded_symbols:
                    sent = ' '.join(sent) + '\n'
                    f.write(sent)
        
        # Print some pred-gold pairs
        import random
        N = len(first_refs)
        random_subsample = [random.randint(0, N-1) for i in range(5)]
        for idx in random_subsample:
            pred, gold = ' '.join(decoded_symbols[idx]), ' '.join(first_refs[idx])
            print("%d-th PRED: %s" % (idx, pred))
            print("%d-th GOLD: %s" % (idx, gold))
...
class EvaluationHook(tf.train.SessionRunHook):
    ...
    def __init__(self, eval_fn, eval_input_fn, eval_decode_fn, base_dir, is_Reverse, is_BPE,
